...
mod integration_tests {
    ...
    async fn test_validation_config_customization() {
        ...
        assert!(custom_validator.validate_node_address("127.0.0.1:8080").is_err());
        
        // Test cluster size restriction
        let large_cluster: Vec<(NodeId, String)> = (1..=5)
            .map(|i| (i, format!("192.168.1.{}:8080", i)))
            .collect();
        assert!(custom_validator.validate_add_node(6, "192.168.1.6:8080", &large_cluster).is_err());
    }
    ...
    async fn test_authorization_integration_stub() {
        ...
        let _result = node.add_node_with_auth(2, "127.0.0.1:8092".to_string(), Some(auth_ctx.clone())).await;
        ...
        let result = node.update_timeouts_with_auth(Some(100), Some(200), Some(400), Some(auth_ctx)).await;
