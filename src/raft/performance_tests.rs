...
mod performance_tests {
    ...
    use crate::raft::{
        node::{RaftNode, NodeConfig, ResourceLimits},
        types::*,
    ...
    async fn benchmark_node_creation_and_startup() {
        ...
        assert!(creation_time < Duration::from_millis(100), "Node creation should be fast");
        ...
        assert!(startup_time < Duration::from_secs(5), "Node startup should be reasonably fast");
        ...
        if leadership_result.is_ok() {
            ...
            assert!(leadership_time < Duration::from_secs(5), "Leadership should be acquired quickly in single-node cluster");
        }
    ...
    async fn benchmark_validation_performance() {
        ...
        let start = Instant::now();
        for i in 1..=10000 {
            let _ = validator.validate_node_id(i);
        }
        let node_id_validation_time = start.elapsed();
        
        println!("10,000 node ID validations time: {:?}", node_id_validation_time);
        println!("Average per validation: {:?}", node_id_validation_time / 10000);
        assert!(node_id_validation_time < Duration::from_millis(100), "Node ID validation should be very fast");
        ...
        for address in &addresses {
            let _ = validator.validate_node_address(address);
        }
        let address_validation_time = start.elapsed();
        
        println!("1,000 address validations time: {:?}", address_validation_time);
        println!("Average per validation: {:?}", address_validation_time / 1000);
        assert!(address_validation_time < Duration::from_millis(500), "Address validation should be fast");
        ...
        println!("100 comprehensive validations time: {:?}", comprehensive_validation_time);
        println!("Average per validation: {:?}", comprehensive_validation_time / 100);
        assert!(comprehensive_validation_time < Duration::from_millis(1000), "Comprehensive validation should be reasonably fast");
    }
    ...
    async fn benchmark_metrics_collection() {
        ...
        println!("1,000 basic metrics collections time: {:?}", basic_metrics_time);
        println!("Average per collection: {:?}", basic_metrics_time / 1000);
        assert!(basic_metrics_time < Duration::from_secs(1), "Basic metrics should be fast");
        ...
        println!("100 comprehensive metrics collections time: {:?}", comprehensive_metrics_time);
        println!("Average per collection: {:?}", comprehensive_metrics_time / 100);
        assert!(comprehensive_metrics_time < Duration::from_secs(1), "Comprehensive metrics should be reasonably fast");
        ...
        assert!(health_check_time < Duration::from_secs(1), "Health checks should be fast");
    ...
    async fn benchmark_resource_limiting() {
        ...
        let start = Instant::now();
        for _ in 0..10000 {
            let _permit = resource_limiter.check_request_allowed(1024, Some("test_client")).await;
        }
        let resource_check_time = start.elapsed();
        
        println!("10,000 resource limit checks time: {:?}", resource_check_time);
        println!("Average per check: {:?}", resource_check_time / 10000);
        assert!(resource_check_time < Duration::from_secs(1), "Resource limit checks should be fast");
        ...
        println!("10,000 concurrent resource checks time: {:?}", concurrent_check_time);
        assert!(concurrent_check_time < Duration::from_secs(5), "Concurrent resource checks should be reasonably fast");
    }
    ...
    async fn benchmark_timeout_configuration() {
        ...
        for i in 0..1000 {
            ...
            let _ = node.update_timeouts(Some(heartbeat), Some(min_timeout), Some(max_timeout)).await;
        }
        ...
        assert!(timeout_update_time < Duration::from_secs(2), "Timeout updates should be fast");
    ...
    async fn stress_test_node_operations() {
        ...
        for _i in 0..50 {
            ...
            let handle = tokio::spawn(async move {
                for j in 0..20 {
                    match j % 4 {
                        0 => { let _ = node_clone.get_metrics().await; },
                        1 => { let _ = node_clone.get_node_health().await; },
                        2 => { let _ = node_clone.get_resource_stats(); },
                        3 => { let _ = node_clone.get_timeout_config(); },
                        _ => unreachable!(),
                    }
        ...
        println!("Stress test (50 tasks × 20 operations) time: {:?}", stress_test_time);
        assert!(stress_test_time < Duration::from_secs(10), "Stress test should complete within reasonable time");
    ...
    async fn memory_usage_benchmark() {
        ...
        assert!(creation_time < Duration::from_secs(5), "Multiple node creation should be efficient");
    ...
    async fn concurrent_validation_benchmark() {
        ...
        for i in 0..100 {
            ...
            let handle = tokio::spawn(async move {
                for j in 1..=100 {
                    ...
                    let _ = validator_clone.validate_node_id(node_id);
                    let _ = validator_clone.validate_node_address(&address);
        ...
        println!("Concurrent validation (100 tasks × 100 validations) time: {:?}", concurrent_validation_time);
        assert!(concurrent_validation_time < Duration::from_secs(2), "Concurrent validation should be fast");
    }
    ...
    async fn large_cluster_simulation_benchmark() {
        ...
        println!("Large cluster validation (1000 nodes + 100 additions) time: {:?}", large_cluster_time);
        assert!(large_cluster_time < Duration::from_secs(1), "Large cluster validation should be efficient");
    }
}